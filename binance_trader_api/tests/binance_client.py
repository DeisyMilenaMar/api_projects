...
class BinanceTraderPriceCheckTestCase(TestCase):
    def setUp(self):
        ...
        self.client_binance = BinanceClient(base_url=self.base_url, timeout=timeout)
    ...
    def test_price_check_error_server(self, mocker):
        mocker.post(
            self.get_price_url, status_code=500, json={"error": "Internal Server Error"}
        )
    ...
    def test_price_check_bad_request(self, mocker):
        mocker.post(self.get_price_url, status_code=400, json={"error": "Bad request"})
