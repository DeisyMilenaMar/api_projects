...
class TransactionCreateViewTests(APITestCase):
    ...
    def test_create_transaction_success(self):
        ...
        response = self.client.post(
            self.url, data=self.valid_transaction_data, format="json"
        )
        json_response = response.json()

        self.assertEqual(response.status_code, status.HTTP_201_CREATED)
        self.assertEqual(
            Decimal(json_response["buy_price"]),
            self.valid_transaction_data["buy_price"],
        )
        self.assertEqual(
            Decimal(json_response["usdt_amount_buy"]),
            self.valid_transaction_data["usdt_amount_buy"],
        )
        self.assertEqual(
            Decimal(json_response["target_profit_percent"]),
            self.valid_transaction_data["target_profit_percent"],
        )
        ...
        self.assertEqual(
            transaction.buy_price, self.valid_transaction_data["buy_price"]
        )
        self.assertEqual(
            (transaction.usdt_amount_buy),
            self.valid_transaction_data["usdt_amount_buy"],
        )
        self.assertEqual(
            (transaction.target_profit_percent),
            self.valid_transaction_data["target_profit_percent"],
        )
...
class UserCreateViewTest(APITestCase):
    ...
    def test_create_user_invalid_email(self):
        ...
        invalid_payload = {"name": "John Doe", "email": "invalid-email"}
...
class UserGetViewTest(APITestCase):
    ...
    def test_get_user_missing_parameters(self):
        ...
        incomplete_params = {"name": "John Doe", "email": "john.doe@example.com"}
