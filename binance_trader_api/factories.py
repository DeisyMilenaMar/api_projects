...
class AdvertiserFactory(DjangoModelFactory):
    ...
    user_type = "company"
    month_finish_rate = Faker(
        "pydecimal", left_digits=1, right_digits=9, positive=True, max_value=1
    )
    positive_rate = Faker(
        "pydecimal", left_digits=1, right_digits=9, positive=True, max_value=1
    )
...
class TransactionFactory(DjangoModelFactory):
    ...
    token_transaction = factory.LazyFunction(
        lambda: generate_token(prefix="trx", with_date=True)
    )
    ...
    buy_date = factory.LazyFunction(
        lambda: timezone.now() - datetime.timedelta(days=30)
    )
    target_profit_percent = Faker(
        "pydecimal", left_digits=1, right_digits=2, positive=True, max_value=1
    )
