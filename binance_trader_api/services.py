...
class PriceMonitoringService:
    ...
    def create_price_check_log(self, transaction: Transaction, request_body: str, response_body: dict, status_code: int):
    ...
    def _process_transaction_price(self, client: BinanceClient, transaction: Transaction):
        ...
        try:
            request_body = json.loads(response.request.body 
                    ...
                    else response.request.body.decode('utf-8') 
                    if hasattr(response, 'request') else '')
        ...
        if price_log.res_http_code != 200:
            logger.error(
                ...
                extra={'response_code': price_log.res_http_code, 'transaction_id': transaction.id}
            )
            return
    ...
    def _analyze_and_notify(self, transaction: Transaction, response_body: dict):
        ...
        target_price = Decimal(transaction.buy_price) * (1 + Decimal(transaction.target_profit_percent))
        try:
    ...
    def _handle_monitoring_error(self, transaction: Transaction, exc: Exception):
        ...
        logger.error(f"Error processing transaction {transaction.id}: {str(exc), type(exc)}")
...
class NotificationService:
    ...
    def send_email_notification(self, transaction: Transaction, advertiser_details):
        ...
        context = {
            ...
            "target_price": transaction.buy_price * (1 + transaction.target_profit_percent),
            ...
            "advertiser_month_finish_rate": advertiser_details.get('month_finish_rate', 0.0),
            "advertiser_positive_rate": advertiser_details.get('positive_rate', 0.0),
            "advertiser_month_order_count": advertiser_details.get('month_order_count', 0)
        }
        ...
        try:
            ...
            logger.error(f"Failed to send email notification for transaction {transaction.id}: {str(exc)}")
