...
class User(AutoCreatedUpdatedMixin):
    ...
    name = models.CharField(
        max_length=60,
        validators=[RegexValidator(regex=r"^[\w á-úÁ-Ú]*$"), MinLengthValidator(2)],
        help_text="User's full name.",
    )
    email = models.EmailField(
        unique=True, help_text="Unique email address used to identify the user."
    )
...
class Advertiser(AutoCreatedUpdatedMixin):
    ...
    user_number = models.CharField(
        max_length=50, help_text="Identification number of the advertiser."
    )
    nickname = models.CharField(max_length=50, help_text="Nickname of the advertiser.")
...
class Advertisement(AutoCreatedUpdatedMixin):
    ...
    AC_BUY = "buy"
    AC_SELL = "sell"
    ACTION_CHOICES = [(AC_BUY, AC_BUY), (AC_SELL, AC_SELL)]
    ...
    ad_number = models.CharField(
        max_length=50, help_text="Unique identifier of the advertisement."
    )
    ...
    asset = models.CharField(
        max_length=50, help_text="Asset being traded (e.g., USDT)."
    )
    fiat_unit = models.CharField(
        max_length=10, help_text="Fiat currency used in the advertisement (e.g., USD)."
    )
    price = models.DecimalField(
        max_digits=15, decimal_places=2, help_text="Price per unit of the asset."
    )
    min_trade_limit = models.DecimalField(
        max_digits=15, decimal_places=2, help_text="Minimum trade limit."
    )
    max_trade_limit = models.DecimalField(
        max_digits=15, decimal_places=2, help_text="Maximum trade limit."
    )
...
class Transaction(AutoCreatedUpdatedMixin):
    ...
    buy_price = models.DecimalField(
        max_digits=10, decimal_places=2, help_text="Price at which the item was bought."
    )
    ...
    notification_sent = models.DateTimeField(
        blank=True, null=True, help_text="Timestamp when notification was sent."
    )
...
class BinanceTraderRequestLog(AutoCreatedUpdatedMixin):
    ...
    req_http_body = JSONField(
        blank=True, null=True, help_text="HTTP request body content, if applicable."
    )
    res_http_body = JSONField(
        blank=True, null=True, help_text="HTTP response body content, if applicable."
    )
    res_http_code = models.IntegerField(
        blank=True, null=True, help_text="HTTP status code from the response."
    )
